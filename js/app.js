...
(function () {
  ...
  function renderCTA() {
    const t = window.I18N && window.I18N.t ? window.I18N : { t: function (k) { return k; } };
    const container = document.getElementById('ctaContent');
    if (!container) return;

    container.innerHTML =
      '<h2>' + escapeHtml(t.t('cta_title')) + '</h2>' +
      '<p>' + escapeHtml(t.t('cta_text')) + '</p>' +
      '<div class="btn-group">' +
        '<a href="#menu" class="btn">' + escapeHtml(t.t('cta_btn1')) + '</a>' +
        '<a href="about.html" class="btn btn-outline">' + escapeHtml(t.t('cta_btn2')) + '</a>' +
      '</div>';
  }
